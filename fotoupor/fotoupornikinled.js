...
app.get('/', function(req, res){
    res.sendFile(path.join(__dirame + '/fotoupornikinled.html'));
});
...
wss2.on('connection', function(ws, req){
    ...
    ws.on('message', function(msgString){ // sporočilo dobimo na stran strežnika kot string, t.j. msgString
        ...
        switch(msg.tipSporočila){
            case "fotoupornik":
                funkcijaFotoupornik(msg); // v primeru, da je tip sporočila iz Chrome klient "fotoupornik" poženemo funkcijo funkcijaFotoupornik()
            break;
        }
...
function funkcijaFotoupornik(msg){ // če je tip sporočila fotoupornik
    wss1.broadcast(JSON.stringify(msg)); // "broad-castamo" po vseh esp-jih - JSON sporočilo posredujemo vsem esp32 modulom
}
